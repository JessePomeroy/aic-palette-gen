...
`palette-usability.mjs` checks the main copy/lock actions with keyboard and touch, successful/denied/pending clipboard operations, repeated-copy feedback, lock preservation during regeneration, and full 5–8 color layouts at 320px, 390px and desktop widths. It measures hex text and hit targets, checks 200% text at 320px, advanced-tool labeling and modal focus restoration, and uses isolated clipboard/museum fixtures plus one native Chromium clipboard check. Run it with `node tests/browser/palette-usability.mjs chromium` (or `webkit`) using the same environment above.
